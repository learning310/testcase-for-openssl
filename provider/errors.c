...
static void err(struct provctx_st *pctx, int reason, const char *file,
		unsigned int line, const char *func, const char *fmt, ...)
{
	va_list ap;
...
#define ERR(pctx, reason, fmt, ...)                         \
	err((pctx), (reason), __FILE__, __LINE__, __func__, \
